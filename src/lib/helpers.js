...
export const T = {
  ...
  accent: '#0066ff', accentLight: '#e8f0ff', accentBorder: '#b3cdff',
  red: '#cc0000', redLight: '#fff0f0', redBorder: '#ffcccc',
...
export function generateWeeks(count = 8) {
  ...
  for (let w = 0; w < count; w++) {
    ...
    weeks.push({ key, label: `Week ${weekNum}`, month: monthNames[weekStart.getMonth()], days })
  }
  return weeks
}
...
export function buildNudgeWhatsApp(activityTitle, appBaseUrl) {
  const title = activityTitle ? ` voor *${activityTitle}*` : ''
  const message = `Hey! Ik wil iets plannen${title} maar er is nog geen beschikbaarheid opgegeven. Kunnen jullie even jullie beschikbaarheid invullen? ${appBaseUrl}`
  return `https://wa.me/?text=${encodeURIComponent(message)}`
}

// Beschikbare datum niet top
export function buildDateNudgeWhatsApp(activityTitle, appBaseUrl) {
  const title = activityTitle ? `*${activityTitle}*` : 'iets'
  const message = `Hey! Ik wil ${title} plannen maar de beschikbare datum vind ik niet top. Kunnen jullie meer beschikbaarheid opgeven? ${appBaseUrl}`
  return `https://wa.me/?text=${encodeURIComponent(message)}`
}

// Groepsbericht met statusoverzicht
...
export function buildInviteWhatsApp(groepNaam, token, appBaseUrl) {
  const message = `Hey! Ik nodig je uit voor onze groep *${groepNaam}* op Wanneer. Plan samen, zonder gedoe: ${appBaseUrl}/join/${token}`
  return `https://wa.me/?text=${encodeURIComponent(message)}`
}
...
export function downloadICS(activity) {
  ...
  const date = activity.best_date?.replace(/-/g, '')
  ...
  const ics = [
    'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Wanneer App//NL',
    'BEGIN:VEVENT',
    `UID:${activity.id}@wanneer-app`,
